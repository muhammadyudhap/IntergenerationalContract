...
%% 
 tauo10vec    = zeros(1,N); %Preallocation for tauo1 generation 0
 tauo11vec    = zeros(1,N); %Preallocation for tauo1 generation 1
 mt           = zeros(1,N); %Preallocation for mitigation
 guesstauo10  = 0.1;
 guesstauo11   = 0.1;
for i = 1:N
    m0s       =(i-1)*di; %the simulated contract for mitigation
    [tauo10vec(i), fval, eflag, out] = fsolve(@(x) solvingdW0(x, parameters, m0s),...
        guesstauo10,optimset('TolX',1.0e-8,'TolFun',1.0e-8,'MaxFunEvals',100000,...
        'MaxIter', 100000));
    guesstauo10 = tauo10vec(i);
    [tauo11vec(i), fval, eflag, out] = fsolve(@(x) solvingdW1(x, parameters, m0s),...
        guesstauo11,optimset('TolX',1.0e-8,'TolFun',1.0e-8,'MaxFunEvals',100000,...
        'MaxIter', 100000));
    guesstauo11 = tauo11vec(i);
    mt(i)  = (i-1)*di;
end;
...
%print(h,'11SubCES_k01_A8_thetamin1_highR1_lowPOL001.pdf','-dpdf','-r0');