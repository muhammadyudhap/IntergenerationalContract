...
for i = 1:N
    ...
    [T10vec(i), fval, eflag, out] = fsolve(@(x) logsolvedW0(x, parameters, m0s),...
        guessT10,optimset('TolX',1.0e-8,'TolFun',1.0e-8,'MaxFunEvals',100000,...
        'MaxIter', 100000));
    [T11vec(i), fval, eflag, out] = fsolve(@(x) logsolvedW1(x, parameters, m0s),...
...
ylabel('Lum-sum Transfer \hspace{0.15cm}$\mathcal{T}_{t+1}$','Interpreter','latex','FontSize',14,'Color','black');
hold off;
set(h,'Units','Inches');
pos = get(h,'Position');
set(h,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]);
%print(h,'Lum_Cobb_R1_lumsuper','-dpdf','-r0');